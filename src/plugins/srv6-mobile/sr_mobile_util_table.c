...
static const u_int8_t mask_bit[] = { 0x00, 0x80, 0xc0, 0xe0, 0xf0,
				     0xf8, 0xfc, 0xfe, 0xff };
...
/* Unlock the node in a given prefix table and then delete the node if the lock
 * is 0 */
int
...
struct sr_table_node *
sr_table_node_base (struct sr_table *table, struct sr_table_node *node,
		    u_int8_t *key, u_int8_t keylen)
{
  int i, j;
  int boundary = 0;
  u_int8_t len;
...
/* Compare the keys */
int
sr_table_node_key_match (u_int8_t *k1, u_int8_t k1len, u_int8_t *k2,
			 u_int8_t k2len)
{
  int offset, shift;
  u_int8_t key, mask;
