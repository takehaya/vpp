  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (ls_param->tedb != NULL)
		...
		{
		  ls_param =
		    (srv6_end_gtp4_d_param_t *) sr_table_node_get_data (node);
		}
	    }

	  if (PREDICT_FALSE ((ls_param == NULL) ||
			     (ls_param->sr_prefixlen == 0)))
	    {
	      next0 = SRV6_T_M_GTP4_D_NEXT_DROP;
	      goto DONE;
	    }
	  ...
	  if (gtp4)
	      ...
	      if (ls_param->sid_present)
		{
		  ...
		  iph->payload_length = clib_host_to_net_u16 (
		    b0->current_length - sizeof (ip6_header_t));
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (ls_param->tedb != NULL)
		...
		{
		  ls_param =
		    (srv6_end_gtp6_d_param_t *) sr_table_node_get_data (node);
		}
	    }
